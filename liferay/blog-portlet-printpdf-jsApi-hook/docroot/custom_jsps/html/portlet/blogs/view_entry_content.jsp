...
function demoFromHTML(element) {
	debugger;
	function getSource(){
		var source = $(element).parent().find('.entry-body')[0]		
		return source;
	}
...
<c:choose>
	<c:when test="<%= BlogsEntryPermission.contains(permissionChecker, entry, ActionKeys.VIEW) && (entry.isVisible() || (entry.getUserId() == user.getUserId()) || BlogsEntryPermission.contains(permissionChecker, entry, ActionKeys.UPDATE)) %>">
		<div class="entry <%= WorkflowConstants.toLabel(entry.getStatus()) %>">
			...
			<button onclick="javascript:demoFromHTML($(this))" class="button ui-button ui-widget ui-state-default ui-corner-all ui-button-text-only" role="button" aria-disabled="false" type="button">Build PDF</button>
