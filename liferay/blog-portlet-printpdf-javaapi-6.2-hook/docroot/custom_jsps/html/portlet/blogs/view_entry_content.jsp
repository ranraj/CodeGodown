...
<c:choose>
	<c:when test="<%= BlogsEntryPermission.contains(permissionChecker, entry, ActionKeys.VIEW) && (entry.isVisible() || (entry.getUserId() == user.getUserId()) || BlogsEntryPermission.contains(permissionChecker, entry, ActionKeys.UPDATE)) %>">
		<div class="entry <%= WorkflowConstants.getStatusLabel(entry.getStatus()) %>" id="<portlet:namespace /><%= entry.getEntryId() %>">
			...
			<portlet:resourceURL var="pdfGenURL">
      			<portlet:param name="struts_action" value="/blogs/view_pdf" />      
      			<portlet:param name="entryId" value="<%= String.valueOf(entry.getEntryId()) %>" />      
     		</portlet:resourceURL>
     		<% String extenstion = "pdf"; %>
  			<div class="export-actions" style="float: right">
								
								<liferay-ui:icon
											image='<%= "../file_system/small/" + extenstion %>'
											label="<%= true %>"
											message='<%= LanguageUtil.format(pageContext, "x-convert-x-to-x", new Object[] {"aui-helper-hidden-accessible", entry.getEntryId(), extenstion.toUpperCase()}) %>'
											method="get"
											url="<%= pdfGenURL.toString() %>"
								/>								
			</div>	
